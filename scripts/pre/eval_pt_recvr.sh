...
#SBATCH --output=./results_extra/recovery-pt-%j-%a-%N.out  # %N for node name, %j for jobID, %a for array ID
...
LOG_DIR_RCV="recovery_gpt2_pt_log"
...
python recovery.py  --model_name $MDL_NAME  --logDIR $LOG_DIR_FT  --modelDIR $MDL_DIR_FT  --logDIR_fgt $LOG_DIR_UNL  --modelDIR_fgt $MDL_DIR_UNL  --logDIR_recvr $LOG_DIR_RCV \
      --unlearnSet $UNLEARN_SET  --datasetType $FORGET_TYPE  --modelType 'pt-unlearned'  \
...
# python recovery.py  --unlearnSet $UNLEARN_SET --datasetType $FORGET_TYPE  --modelType 'pt-unlearned'  \
