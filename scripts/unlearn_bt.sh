...
#SBATCH --output=./results_extra/unlearn-bt-%j-%a-%N.out  # %N for node name, %j for jobID, %a for array ID
...
accelerate launch --multi_gpu unlearn.py --model_name deepseek-ai/deepseek-llm-7b-chat \
    --finetune_model_DIR fine_tuned_deepseek_7b_bt  --logDIR unlearn_deepseek_7b_bt_log --unlearn_model_DIR unlearn_deepseek_7b_bt \
    --unlearnSet $UNLEARN_SET --forgetSetDir $FORGET_SET --retainSetDir $RETAIN_SET \
    --lr_ft 0.0005  --eps_ft 50 --wd_ft 0.01  --LoRA_rank_ft 256  --lora_dropout_ft 0.0 --grad_acc_steps_ft 40 \
