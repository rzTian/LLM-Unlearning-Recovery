...
#SBATCH --output=./results_extra/recovery-cost-%j-%a-%N.out  # %N for node name, %j for jobID, %a for array ID
...
wd_list=(0.01)
lr_list=(0.0002)
recvr_list=('hf_greedy' 'manual_greedy' 'flip' 'beam')
set_list=("unlearn-N40-A1-bld10" "unlearn-N40-A1-sin10" "unlearn-N40-A1-yrb10" "unlearn-N40-A1-pcd10")
...
epoch_list=(28 32 36 40 44 48) # 4 8 12 16 20 24 
...
LOG_DIR_UNL="unlearn_deepseek_7b_log-arch"
MDL_DIR_UNL="unlearn_deepseek_7b-arch"
LOG_DIR_RCV="recovery_deepseek_7b_log-arch"
...
cd $HOME/links/projects/rrg-yymao/hsc/LLM-Unlearning-Recovery

echo "当前测试: Unlearn Set: $UNLEARN_SET | Forget Type: $FORGET_TYPE | $RECOVER_TYPE 0 "
python recovery_cost.py  --model_name $MDL_NAME  --logDIR $LOG_DIR_FT  --modelDIR $MDL_DIR_FT  --logDIR_fgt $LOG_DIR_UNL  --modelDIR_fgt $MDL_DIR_UNL  --logDIR_recvr $LOG_DIR_RCV \
...
if [[ "$RECOVER_TYPE" == "flip" || "$RECOVER_TYPE" == "beam" ]]; then
  ...
  echo "跳过 $RECOVER_TYPE flip=1：该 baseline 与 flip 无关，避免重复覆盖同一 cost 文件。"
fi
