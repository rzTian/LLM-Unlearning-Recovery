...
#SBATCH --output=./results/tofu/logs/05-recovery-%j-%a-%N.out
#SBATCH --job-name=tofu-05-recovery
...
RESULTS_ROOT=${RESULTS_ROOT:-results/tofu/recovery}
...
fail_missing_adapter() {
  local path="$1"
  echo "[05-recovery][ERROR] missing adapter path" >&2
...
python -u tofu/05_eval_recovery.py "${ARGS[@]}"
